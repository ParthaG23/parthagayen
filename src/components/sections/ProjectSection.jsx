...
export default function ProjectsPreview({ dark }) {
  ...
  return (
    <section
      ...
      className={`py-24 lg:py-32 transition-colors duration-500 ${dark ? "text-white" : "text-gray-900"}`}
      style={glassStyle}
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <h2 className="text-3xl lg:text-5xl font-extrabold mb-4">SELECTED PROJECTS</h2>
          <p className="opacity-70 max-w-2xl mx-auto text-sm lg:text-base">Some of my recent real-world applications.</p>
        </motion.div>
          ...
          {recentProjects.map((project, index) => {
            ...
            return (
              <div key={project.id} className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                ...
                >
                  <p className="text-xs uppercase tracking-widest text-lime-400 mb-3">Featured Project</p>
                  <h3 className="text-2xl lg:text-3xl font-bold mb-4">{project.title}</h3>
                  <p className="opacity-70 mb-6 leading-relaxed text-sm lg:text-base">{project.description}</p>
                  <div className="flex flex-wrap justify-center lg:justify-start gap-3 mb-8">
                    {project.tech.map((tech, i) => (
                      <span key={i} className="px-3 py-1 text-xs lg:text-sm rounded-full border border-lime-400/30 text-lime-400">{tech}</span>
                    ))}
                  </div>
        ...
        <div className="text-center mt-20 lg:mt-28">
          <Link to="/projects" className="inline-block px-8 lg:px-12 py-3 lg:py-4 rounded-full border-2 border-lime-400 text-lime-400 text-sm lg:text-base font-semibold hover:bg-lime-400 hover:text-black transition-all duration-300">
            View All Projects
          </Link>
        </div>
